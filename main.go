...
func (s *Server) handleTryOn(w http.ResponseWriter, r *http.Request) {
	...
	personJPEG, err := convertToJPEG(personData)
	if err != nil {
		log.Printf("Failed to convert person image: %v", err)
		sendError(w, "人物画像の処理に失敗しました", http.StatusInternalServerError)
		return
	}

	garmentJPEG, err := convertToJPEG(garmentData)
	if err != nil {
		log.Printf("Failed to convert garment image: %v", err)
	...
	ctx := context.Background()
	result, err := s.callVirtualTryOn(ctx, personJPEG, garmentJPEG)
	if err != nil {
		log.Printf("Virtual Try-On failed: %v", err)
		hint := "ヒント: 露出や著名人・ロゴ類・過度な加工を避け、人物と衣服がはっきり写る画像で再試行してください。"
		sendError(w, fmt.Sprintf("生成に失敗しました: %v %s", err, hint), http.StatusInternalServerError)
		return
	}
